...
class Response:
    def __init__(self, body=b"", status_code=200, headers=None):
        ...
        # Normalize headers to a list so duplicate header names are preserved.
        if headers is None:
            normalized_headers = []
            ...
            try:
                ...
                raise ApplicationError(
                    "Response headers must be a dict or an iterable of "
                    "(name, value) pairs."
                ) from error
        ...
        for header in normalized_headers:
            ...
            if "\r" in name or "\n" in name:
                raise ApplicationError(
                    "Response header name must not contain carriage return or "
                    "line feed characters."
                )

            if "\r" in value or "\n" in value:
                raise ApplicationError(
                    "Response header value must not contain carriage return or "
                    "line feed characters."
                )

            if name.lower() == "content-length":
                raise ApplicationError(
                    "Content-Length header is managed by the server and must not "
    ...
    def to_bytes(self):
        ...
        # Work on a copy so repeated serialization does not mutate the response.
