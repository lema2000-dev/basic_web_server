...
class Server:
    def __init__(self, application, config=None):
        ...
        self.config = config
        self.logger = create_logger(self.config.log_file, self.config.log_level)
        ...
        # Console commands are kept in one registry so help stays in sync.
        self._commands = {
            ...
            "quit": (
                self._command_quit,
                "Stop the server (if running) and exit the console",
            ),
            "clear": (self._command_clear, "Clear the console screen"),
            "logs": (self._command_logs, "Show server logging configuration"),
            "version": (
                self._command_version,
                "Show server version and Python version",
            ),
        }
    ...
    def _server(self):
        try:
            ...
            while self._running:
                ...
                # Register before start so very short-lived threads can remove themselves.
                ...
                try:
                    ...
                    self.logger.error(
                        "Failed to start client thread for %s: %s",
                        client_address,
                        error,
                    )
                    ...
                    with self._client_threads_lock:
                        self._client_threads = [
                            (thread, address)
                            for thread, address in self._client_threads
                            if thread is not client_thread
                        ]
            ...
            with self._client_threads_lock:
                client_threads_copy = [
                    thread for thread, _ in self._client_threads
                ]
    ...
    def _command_clients(self):
        ...
        print(f"Active client connections: {len(client_threads)}")
    ...
    def _command_logs(self):
        ...
        print("Server logging configuration:")
        print(f"  Log level: {log_level}")
        print(f"   File: {self.config.log_file}")

    def _command_version(self):
        print("Basic Web Server version:")
